...
data_geom <-st_read("C:/Users/hangzh/Downloads/Lecture 1 - RegressionData.shp/RegressionData.shp")
...
par(mfrow=c(2,2))
plot(data_cor$LNNBELPOV, data_cor$LNMEDHVAL)
plot(data_cor$PCTBACHMOR, data_cor$LNMEDHVAL)
plot(data_cor$PCTVACANT, data_cor$LNMEDHVAL)
plot(data_cor$PCTSINGLES, data_cor$LNMEDHVAL)
```
...
summary(fit1)
cv <- CVlm(data = data_cor, fit1, m=5)
mse <- attr(cv, "ms")
...
summary(fit2)
cv <- CVlm(data=st_drop_geometry(data_geom), fit2, m=5)
mse <- attr(cv, "ms")
...
## Model Quality - Jiahang
The results from the three referenced models consistently indicate that they are high-quality models overall. After assessing the regression model, the output (the F-statistic is high, and the P-value is way much smaller than 0.05) suggests the strength of the regression model. To be more specific, the high F-statistic means that we can reject the null hypothesis that none of the independent variables in the model is a significant predictor of the dependent variable. Also, the multiple R-square (0.6623) indicates that approximately 66.23% of the variance in median house values can be explained by the model. 

Also, in the stepwise regression results, all 4 predictors in the original model are retained in the final model, which means that the original model is the best model. That is to say, all variables have a statistically strong relationship with the dependent variables. 

Moreover, based on the output of the cross-validation model, the original model, which considers all four variables, has a lower RMSE, 0.366 compared to the model solely relying on 'housing vacancy' and 'median household income' as predictors, which yields an RMSE of 0.443. This underscores the superiority of the original model.


The output from the models all suggests that there is a strong relationship between median household value and residents’ educational level, housing vacancy, percent of housing units that are detached single-family houses, and median household income. To elaborate further, housing vacancy rates and the percentage of detached single-family houses serve as indicators of housing quality and pricing dynamics. Simultaneously, household income and residents' education levels are closely tied to economic status and the purchasing capacity of prospective homeowners. However, there is potential to enhance the model's comprehensiveness by introducing additional variables. For instance, the age of the housing stock exerts a profound influence on housing prices, and the number of bedrooms within a property can also significantly impact its market value. These considerations underscore the opportunity for enriching the model with a more comprehensive set of predictors.
